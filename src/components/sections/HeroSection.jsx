...
import { Search, ArrowRight, BookOpen, Users, PlayCircle, Award } from 'lucide-react'
...
export default function HeroSection() {
  ...
  const videoCount = channel?.statistics?.videoCount ? Number(channel.statistics.videoCount) : 500
  ...
  return (
    <section className="relative min-h-[92vh] flex flex-col justify-center bg-ocean-950 overflow-hidden">
      ...
      {/* Animated blobs */}
      <div className="absolute top-[-10%] right-[-5%] w-[500px] h-[500px] rounded-full opacity-[0.07] blur-[80px]"
           style={{ background: 'radial-gradient(circle, #00b4d8, #0077b6)' }} />
      <div className="absolute bottom-[-5%] left-[-5%] w-[400px] h-[400px] rounded-full opacity-[0.06] blur-[60px] animate-float"
           style={{ background: 'radial-gradient(circle, #48cae4, #0096c7)' }} />
      <div className="absolute top-[40%] left-[30%] w-[300px] h-[300px] rounded-full opacity-[0.04] blur-[50px]"
      ...
      <div className="section relative z-10 py-20 md:py-28">
        <div className="max-w-4xl mx-auto text-center">
          ...
          <div className={`flex justify-center mb-10 transition-all duration-700 ${visible ? 'opacity-100 translate-y-0' : 'opacity-0 -translate-y-4'}`}>
            <div className="relative">
              <img
                ...
                onError={e => {
                  ...
                  svgImg.onload = () => { e.currentTarget.src = '/ocw-logo.svg'; e.currentTarget.className = 'h-16 w-auto object-contain' }
                  svgImg.onerror = () => { e.currentTarget.parentElement.style.display = 'none' }
          ...
          <h1 className={clsx_light(
            ...
            <span className="relative inline-block">
              ...
              <svg className="absolute -bottom-2 left-0 w-full" viewBox="0 0 300 8" fill="none">
                <path d="M2 6 Q75 1 150 5 Q225 9 298 3" stroke="url(#ul)" strokeWidth="2.5" strokeLinecap="round" fill="none"/>
                <defs><linearGradient id="ul" x1="0%" y1="0%" x2="100%" y2="0%"><stop offset="0%" stopColor="#48cae4"/><stop offset="100%" stopColor="#90e0ef"/></linearGradient></defs>
              </svg>
            </span>
          </h1>

          <p className={clsx_light(
          ...
          {/* CTAs */}
          <div className={clsx_light(
            'flex flex-wrap gap-3 justify-center mb-16 transition-all duration-700 delay-400',
            visible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-4'
          )}>
            <Link to="/courses" className="btn-primary btn-lg gap-2 animate-pulse-ring">
              Browse Courses
          ...
          <div className={clsx_light(
            ...
            {stats.map(({ icon: Icon, label, value, suffix }) => (
              <div key={label} className="glass rounded-2xl p-5 text-center group hover:bg-white/15 transition-all">
                <Icon className="w-5 h-5 text-ocean-400 mx-auto mb-2" strokeWidth={1.8} />
...
function clsx_light(...args) { return args.filter(Boolean).join(' ') }
