...
export default function SearchBar({ placeholder = 'Search courses, topics…', className = '', size = 'md' }) {
  ...
  return (
    <form onSubmit={handleSubmit} className={clsx('relative w-full min-w-0', className)}>
      ...
      <input type="search" value={query} onChange={e => setQuery(e.target.value)}
             ...
             className={clsx('w-full min-w-0 rounded-2xl border border-slate-200 dark:border-white/10 bg-white dark:bg-night-200 shadow-card font-body text-ink dark:text-white focus:outline-none focus:ring-2 focus:ring-ocean-400/40 focus:border-ocean-400 transition-all', padX)} />
      <button type="submit"
